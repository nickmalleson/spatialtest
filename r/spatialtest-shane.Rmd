...
spatialtest <- function(basepoints, testpoints, areas, get.monte.carlo=FALSE) {
  ...
  areas$base <- poly.counts(basepoints, areas)
  areas$test <- poly.counts(testpoints,areas)

  len <- length(areas) # The number of areas
  tot <- sum(areas$test)    # The number of crimes
  ...
  ct <- 0 # counter
  for (i in 1:len) {
    # XXXX Is there a bug here?? If there are no crimes in area i then end<start. I'll experiment
    # with this in the latter implementation. ONCE I'VE FIXED THE PROBLEM, COME BACK HERE
    # AND GET RID OF THIS TEXT OR CHANGE THE CODE SO THAT IT IS CORRECT
    start <- ct + 1
    #print(as.character(areas$CTUID[i]))
    #print(paste(i, ct, start, areas$test[i], end, sep=" "))
    end <- start + areas$test[i] - 1 # Number of crimes in area i
  ...
  for (i in 1:N) { # (Could do this bit multi-process)
    ...
    # Create a random number for each row drawn from a uniform distribution
    mc.samp <- trunc(runif(tot,min = 1,max = (tot + 1))) 
    result[2,(1:tot)] <- result[1,mc.samp[1:tot]] # Store sampled data in result[2,] temporarily
    
    # For each area, reaggregate
    for (j in 1:len) { 
  ...
  for (i in 1:len) {
    ...
    monte.carlo[(N + 2),i] <- temp[0.975 * N] #upper
    monte.carlo[(N + 3),i] <- temp[0.025 * N] #lower
  }
  ...
  areas@data$Local.S <- ifelse(
    areas$base <= areas$conf.lower, -1,
       ifelse( areas$base >= areas$conf.upper, 1, 0 )
    )
  #areas@data$Local.S <- ifelse(
  ...
  S <-  sum(
    (areas$test >= areas$conf.lower) & (areas$test <= areas@data$conf.upper)
    ) / len 
  ...
  S.nozero <- sum(
    (areas$test >= areas$conf.lower) & (areas$test <= areas@data$conf.upper) & (areas$test > 0)
    ) / len 
...
```{r A-plotSamples}
  ...
  plot(
    result@data$mean, result$test, xlab = "Resampled Counts", ylab = "Observed Count",
    main="All samples and their mean for each area in the base data set"
  )

  # Plot all of the samples
  for (i in 1:N) { 
    points(monte.carlo[i,],result$test, col = "grey", pch = 16, cex = 0.5)
  }
  # And add the means back in again (otherwise they are hidden by the samples)
  points(result@data$mean, result$test, pch = 16, col = "black", cex = 0.5, )
  
  # Add a legend
  legend(x=0, y=max(result$test), c('Mean','Samples'), pch=16, col=c("black","grey") )

```

Plot the mean, the confidence intervals, and the actual test number of points for each area

```{r A-plotConfInt}

  # Upper confidence interval first
  plot(
    result$conf.upper, result$test, 
  ...
  points(result$conf.lower, result$test,pch = 16,col = "dark grey",cex = 0.5)
  
  # Mean
  points(result$mean, result$test, pch = 16,col = "black",cex = 0.5)
...
```{r A-plotConfIntWithTest}
  ...
  # Base data - blue for within intervals, red outside
  points(
    result$base, result$test, pch = 16, cex = 0.5,
    col=ifelse(result$base<result$conf.upper & result$base>result$conf.lower, "green", "red") 
  )
  
  legend(x=0, y=max(result$base), 
         c('Mean','Confidence intervals', 'Test value within intervals', 'Test value outside intervals'), 
         pch=16, col=c("black","grey","green", "red"), cex=0.8)
