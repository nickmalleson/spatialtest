...
```{r config}
datasource <- "./data/" # THe folder with the shapefiles
...
```{r runTest}
  ...
  for (i in 1:N) { # (Could do this bit multi-process)
    ...
    mc <- trunc(runif(tot,min = 1,max = (tot + 1))) # Create a random number for each row
    result[2,(1:tot)] <- result[1,mc[1:tot]] # Store sampled data in result[2,] temporarily
    for (j in 1:len) {
